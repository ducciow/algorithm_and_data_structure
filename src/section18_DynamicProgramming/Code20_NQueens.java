...
public class Code20_NQueens {
    ...
    public static int process2(int limit, int colX, int leftDiagX, int rightDiagX) {
        ...
        int ans = 0;
        // 1 in validPos means valid positions
        int validPos = limit & (~(colX | leftDiagX | rightDiagX));
        int mostRightOne = 0;
        while (validPos != 0) {
            // pick the rightmost valid position
            mostRightOne = validPos & (~validPos + 1);
            // mark that position accordingly
            validPos -= mostRightOne;
