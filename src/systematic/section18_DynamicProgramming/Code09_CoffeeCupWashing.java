 ...
 * @Author: duccio
 * @Date: 26, 04, 2022
 * @Description: Two sub-tasks: 1. There are several coffee machines having different periods of time to get a cup of
 *      coffee ready. Here comes some people using those machines for coffee drinking. Return the arrangement that all
 *      the people have their coffee at the earliest time. Each machine is making one cup of coffee each time, and once
 *      the machine finishes one cup it can directly start to make another. 2. There is one cup washing machine that
 *      can wash one coffee cup at one time. When a person finishes the drinking, he can decide to use the washing machine
 *      or leave the cup to evaporate itself. Given the array of time people finish their drinking, return the earliest
 *      time all coffee cups become clear.
 * @Note:   For sub-task1, use a min heap storing all coffee machines based on the sum time of their current starting
 *              point and working time. Arrange each coming person to the polled machine.
 *          ======
 *          For sub-task2, Ver1. brute force according to whether the current cup is washed by machine.
 *                         Ver2. DP.
 *          ======
 *          For sub-task2, it is attempt with task limit, ie., the readyToWash time point varies. In this case, use the
 *                         maximum limit as the dp table size. Besides, when filling in the dp table, early stop if it
 *                         meets unrealistic cells, eg., the readyToWash exceeds the maximum possible upper limit.
 */
public class Code09_CoffeeCupWashing {
    ...
    public static int minTime1(int[] coffeeMakingTime, int numPeople, int washTime, int evaporateTime) {
        PriorityQueue<Machine> pq = new PriorityQueue<>((m1, m2) -> ((m1.start + m1.working) - (m2.start + m2.working)));
        ...
        for (int n = 0; n < numPeople; n++) {
            ...
            machine.start += machine.working;
            serve[n] = machine.start;
            pq.add(machine);
        }
